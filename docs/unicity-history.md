...
Nevertheless, the formula is *additionally* faulty as it does not measure the keyspace correctly. The plaintext is not a discrete sequence of the 48 sound values indicated by YD (48 is again a subjective number - linguists have identified upto 64 sound values in Sanskrit, but let's accept this for the moment), and the rules by which they combine introduces a lot more equivocation.
